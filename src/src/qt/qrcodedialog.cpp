...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non OMC unit input in lnReqAmount, we generate the URI with OMC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(OmnicoinUnits::format(OmnicoinUnits::OMC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
