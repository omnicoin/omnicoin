...
const int _IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString _IPC_PREFIX("omnicoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        socket->waitForBytesWritten(_IPC_CONNECT_TIMEOUT);
